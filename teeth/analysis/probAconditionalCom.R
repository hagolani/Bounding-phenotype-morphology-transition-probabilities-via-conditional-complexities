...
df0=read.table("data/allbombs.dat",header = T, row.names = NULL)

df0 = df0[,-1] 
colnames(df0)=c("Cusps","OPC","ConditionalComplexityCusps","ConditionalComplexityOPC","Freq_Cusps","Freq_OPC","Parent")

df0.ori=df0

for (opcorcusp in 1:2) {
  #OPC
 if(opcorcusp==1)df0=data.frame(Parent=df0.ori$Parent,Mutant=df0.ori$OPC,ConditionalComplexity=abs(df0.ori$ConditionalComplexityOPC),Freq=df0.ori$Freq_OPC,pheno=df0.ori$OPC)
 if(opcorcusp==2)df0=data.frame(Parent=df0.ori$Parent,Mutant=df0.ori$Cusps,ConditionalComplexity=abs(df0.ori$ConditionalComplexityCusps),Freq=df0.ori$Freq_Cusps,pheno=df0.ori$OPC)

 if(opcorcusp==1)comtype="OPC"
 if(opcorcusp==2)comtype="Cusps"


df0$OPpaI= df0$Parent
df0$OPmuI= df0$Mutant
...
for (indi in allindis) {
    ...
    df1=df1[df1$Freq>-3.5,]
    
    if(opcorcusp==1){
    # Identify outliers in the x column using the IQR
    Q1 <- quantile(df1$ConditionalComplexity, 0.25)  # First quartile (25%)
    Q3 <- quantile(df1$ConditionalComplexity, 0.75)  # Third quartile (75%)
    ...
    #filtered_df1 <- subset(df1, ConditionalComplexity >= lower_bound & ConditionalComplexity <= upper_bound)
    filtered_df1 <- subset(df1, ConditionalComplexity <= upper_bound)
    
    df1 = filtered_df1
    }
    
    NPheno=length(unique(df1$pheno))
    #NPheno=length(df1$Mutant)*0.5
    print(NPheno)
    ...
    interv = maxkc / 10
    if(indi==1){maxis=data.frame(Max_ckc=df1$ConditionalComplexity,Max_freq=df1$Freq)
               maxis=maxis[-1,]
    }else{
    maxis=data.frame()
    ...
    pdf(paste(indi,"_c_shade_CI_",comtype,".pdf",sep=""))
    ...
    par(cex.axis = cex_axis, cex.lab = cex_lab , cex_main=cex_lab)
     ...
     legend("topright",legend = c("Fitted model","Fitted 95% CI","Bound model"),
      ...
      tmp.cc=df0[which(df0$Parent==indi),]
      #paI=tmp.cc$OPpaI[1]
      #tmp.cc2=tmp.cc[tmp.cc$OPmuI==paI,]
      #Ncount=nrow(tmp.cc2)
      Ncount=nrow(tmp.cc)
    ...
    pdf(paste(indi,"_c_shade_CI_stats_",comtype,".pdf",sep=""))
    par(mar = c(5, 6, 4, 2) + 0.1)  # Adjusting margin
    par(cex.axis = cex_axis, cex.lab = cex_lab , cex_main=cex_lab)
...
write.table(save.stas,paste("data/stats_v2_",comtype,".dat",sep=""))
...
save.stas=read.table(paste("data/stats_v2_",comtype,".dat",sep=""))
par(mfrow=c(1,1))



pdf(paste("stats_",comtype,"_corr_v2.pdf",sep=""))
par(mar = c(5, 6, 4, 2) + 0.1)  # Adjusting margin
...
pdf(paste("stats_",comtype,"_pear_r2_v2.pdf",sep=""))
par(mar = c(5, 6, 4, 2) + 0.1)  # Adjusting margin
par(cex.axis = 2.2, cex.lab = 2.2, cex.main = 1)
...
pdf(paste("stats_",comtype,"_lm_v2.pdf",sep=""))
par(mar = c(5, 6, 4, 2) + 0.1)  # Adjusting margin
par(cex.axis = 2.2, cex.lab = 2.2, cex.main = 1)
...
pdf(paste("stats_",comtype,"_slope_v2.pdf",sep=""))
par(mar = c(5, 6, 4, 2) + 0.1)  # Adjusting margin
par(cex.axis = 2.2, cex.lab = 2.2, cex.main = 1)
