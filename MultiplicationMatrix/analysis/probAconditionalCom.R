...
dfmax=read.table("data/in_out_count.txt")
NPheno=length(unique(dfmax$V2)) # mmax , uniqM
rm(dfmax)

df=read.table("data/results_multmatrix.dat",header = T)
allindis=unique(df$Parent)

nindis=as.data.frame(table(df$Parent))
...
for (indi in allindis) {
    ...
    par(cex.axis = cex_axis, cex.lab = cex_lab , cex_main=cex_lab)
     ...
     legend("topright",legend = c("Fitted model","Fitted 95% CI","Bound model"),
      ...
      Ncount=nindis[nindis$Var1==indi,2]
